...
watch(
  ...
  async (roomId) => {
    ...
    // Clear DynamicScroller's height cache from the previous room.
    // Without this, recycled DOM nodes inherit stale heights from the old room's
    // messages, causing wrong position offsets and visible gaps.
    (scrollerRef.value as any)?.forceUpdate?.(true);
    loading.value = false;

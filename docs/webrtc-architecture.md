...
## Известные проблемы (март 2026)

> Подробный план исправлений: [webrtc-fix-plan.md](./webrtc-fix-plan.md)
> Анализ логов: [webrtc-logs-analysis.md](./webrtc-logs-analysis.md)

### Асимметрия направлений звонков

Звонки **Mobile → Mac** (answerer/polite) иногда проходят, а **Mac → Mobile** (offerer/impolite) — стабильно падают. Причина: после ICE failure SDK делает ICE restart, caller (impolite) шлёт новый offer, но **игнорирует** ответный negotiate от callee как "colliding". Результат — deadlock.

Место в SDK: `matrix-js-sdk-bastyon/src/webrtc/call.ts`, метод `onNegotiateReceived()`, строка ~1976:
```typescript
...
TURN credentials запрашиваются через SDK (`client.getTurnServers()`), но:
- Не подтверждено, что сервер `matrix.pocketnet.app` корректно отдаёт TURN URIs
- Отсутствует TURNS (TLS) на порт 443 — корпоративные сети не могут использовать TURN
- `fallbackICEServerAllowed: true` включает только базовый STUN

### ICE candidate race condition

`m.call.candidates` может прийти через Matrix /sync раньше `m.call.answer`. Метод `addIceCandidates()` в SDK не проверяет наличие `remoteDescription` перед вызовом `peerConn.addIceCandidate()` → `InvalidStateError`. Также баг буферизации: `odid !== odid` всегда false.

### Утечка таймера iceReconnectionTimeOut

При `iceConnectionState → disconnected` SDK ставит таймер `restartIce()` (2с). При восстановлении в `connected` таймер **не очищается** → ненужный restart дестабилизирует рабочее соединение.

### Отсутствие обработки потери треков

Нет подписки на `track.onended` для локальных медиа-треков. При отключении устройства (USB-микрофон) звонок продолжается с тишиной без уведомления.
